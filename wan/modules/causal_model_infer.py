...
def block_relativistic_rope(x, grid_sizes, freqs, start_frame=0, relative_frame_indices=None):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        if relative_frame_indices is not None:
            # Only use the first f indices for this sample's frame count
            frame_indices = relative_frame_indices[:f].long()
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
                ...
                cache_grid_sizes[:, 0] = num_cache_frames
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        block_mask, kv_cache=None, crossattn_cache=None, current_start=0,
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    Inference-only version with Block-Relativistic RoPE.
    """
    ...
    def _prepare_blockwise_causal_attn_mask(
        ...
        frame_indices = torch.arange(start=0, end=total_length,
    ...
    def _prepare_teacher_forcing_mask(
        ...
        frame_indices = torch.arange(start=0, end=num_frames * frame_seqlen,
    ...
    def _prepare_blockwise_causal_attn_mask_i2v(
        ...
        frame_indices = torch.arange(start=frame_seqlen, end=total_length,
                                     step=frame_seqlen * num_frame_per_block, device=device)
    ...
    def _apply_cache_updates(self, kv_cache, cache_update_infos):
        """Apply cache updates collected from multiple blocks.
        Stores UN-ROPED K values in the cache. RoPE is applied dynamically during attention."""
    ...
    def _forward_train(self, *args, **kwargs):
        raise NotImplementedError(
            "InferCausalWanModel is inference-only. Use CausalWanModel for training.")
...
# Alias for backward compatibility with wan_wrapper.py import
InferCausalWanModel = CausalWanModel
