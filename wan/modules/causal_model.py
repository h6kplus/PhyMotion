...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            # KV cache branch: Apply RoPE BEFORE storing to cache (LongLive approach)
            # This avoids RoPE dimension mismatch when start_frame + f exceeds RoPE table size
            frame_seqlen = math.prod(grid_sizes[0][1:]).item()
            current_start_frame = current_start // frame_seqlen

            # Apply RoPE to q and k before cache operations
            roped_query = causal_rope_apply(q, grid_sizes, freqs, start_frame=current_start_frame, scene_cut=scene_cut).type_as(v)
            roped_key = causal_rope_apply(k, grid_sizes, freqs, start_frame=current_start_frame, scene_cut=scene_cut).type_as(v)
            ...
            if self.local_attn_size != -1 and (current_end > kv_cache["global_end_index"].item()) and (
                ...
                # Construct temporary k, v for computation
                ...
                # Insert new key/value into the temporary cache (with RoPE already applied)
                ...
                # Save cache update info for later use (store roped_key for cache update)
                cache_update_info = {
            ...
            # Compute attention using already-roped query and cached keys (also roped)
            # No need for dynamic RoPE computation at attention time
            if sink_tokens > 0:
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_result = self.self_attn(
            ...
            seq_lens, grid_sizes,
            freqs, block_mask, kv_cache, current_start, cache_start, sink_recache_after_switch, scene_cut)
...
class CausalWanModel(ModelMixin, ConfigMixin):
    ...
    def _set_gradient_checkpointing(self, module=None, value=False, enable=False):
        self.gradient_checkpointing = value or enable
